...
<head>
    ...
    $(function() {
    	$('#btn_submit').click(function(){
    		if($('#editForm').valid()){
	    		$.post('<%=basePath%>resource/doResourceEdit', $('#editForm').serialize(), function(data){
...
<body>
    ...
    <div class="content">
        <table border="0" cellpadding="0" cellspacing="0" class="table">
            <tr>
                <td width="120px" align="right" height="25px">资源名称：</td>
                <td><input type="text" name="name" value="${resource.name}" style="width:200px;" class="{required:true,maxlengthCN:50}"/></td>
            </tr>
            <tr>
                <td align="right" height="25px">资源类型：</td>
                <td>
                	<select name="type" style="width:200px;" class="{required:true}">
                		<option value="left-1">左侧一级菜单</option>
                		<option value="left-2">左侧二级菜单</option>
                	</select>
                </td>
            </tr>
            <tr>
                <td align="right" height="25px">父资源：</td>
                <td>
                	<select name="parentId" style="width:200px;" class="{required:true}">
                		<option value="0">顶级资源</option>
                		<c:forEach items="${resources}" var="parent">
                		<option value="<c:out value="${parent.id}"/>"><c:out value="${parent.name}"/></option>
                		</c:forEach>
                	</select>
                </td>
            </tr>

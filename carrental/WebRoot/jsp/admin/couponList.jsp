...
<html>
		...
		<script type="text/javascript">
			$(function(){
				...
				$("#clearForm").click(function(){
					$("#coupon_couponTitle").val('');
					$("#coupon_couponType").val('');
				});
				//查询
				...
				//新增
				$("#add").click(function(){
					$.show('新增优惠活动','<%=basePath%>coupon/toCouponEdit?id=',550,400,"A");
				});
				//修改
				$("#update").click(function(){
					var ids = getSelectedIdArray();
					if(ids.length==1){
						$.show('修改优惠活动','<%=basePath%>coupon/toCouponEdit?id='+ids[0],550,400,'A');
					}else{
						$.prompt('请选择一条数据',{
							title: '提示',
		        			buttons: { "确认": false}
			...
			function showDetail(id){
				$.show('优惠活动详细信息','<%=basePath%>coupon/toCouponDetail?id='+id,550,400,'A');
			}
		</script>
	</head>
	<body>
	...
	<div class="button_nde">
		优惠活动标题：<input type="text" id="coupon_couponTitle" name="couponTitle" value="${coupon.couponTitle}" class="input"/>
	...
	<div class="content">
		<table width="100%" border="0" cellspacing="1" cellpadding="0" class="show">
			...
			<tr>
				...
				<td name="couponTitle" align="center"><a href="javascript:void(0);" style="color: blue;" onclick="showDetail('${parent.id}')"><c:out value="${parent.couponTitle}"/></a></td>
